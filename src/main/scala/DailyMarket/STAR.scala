...
class STAR(override val date: String, spark: SparkSession) extends StocksDailyBasic(date, spark) {

  override val file_name: String = "STAR"

  df = df_daily_all.filter("sector == 'STAR'")
